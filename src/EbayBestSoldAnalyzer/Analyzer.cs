    ...
    public class Analyzer
    {
        ...
        public async Task<List<Item>> Run(string url)
        {
            ...
            for (int i = 200; i < 5000; i += 200)
            {
                ...
                string urlNew = url + "&_pgn=" + numPage + "&_skc=" + i + "&rt=nc";
                var response = await httpClient.GetAsync(new Uri(urlNew));
                ...
                foreach (var element in listElement.Where(e => e.Descendants().Where(n => n.GetAttributeValue("class", "").Equals("hotness-signal red")).Count() > 0))
                {
                    ...
                    item.Image = element.Descendants("img").FirstOrDefault().GetAttributeValue("src", "");
